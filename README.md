# Задания и вариант решения 2 этапа конкурса "Администрирование Linux" IT-планеты
...
### <a name='perms'></a>3. Права пользователей
...
### <a name='docker'></a>6. Установить и настроить docker
...
Я не делал скрипт сложнее и не добавлял проверок на существование юзеров, так как этого не было указано в ТЗ (нет, я просто ленивый).
...
Заходим в контейнер и проверяем что `sshd` есть в автозагрузке, заодно и узнаем адрес контейнера:
```
...
### <a name='docker_my'></a>6. Установить и настроить docker
...
Тут все просто, атрибут неизменяемости (**i**mmutable) стоит на файле, убираем его и переносим файл:
```
# lsattr ohmyfile 
----i----------- ohmyfile
# chattr -i ohmyfile 
