...
* первый этап, ответы на теоретические вопросы разной сложности (20-30 штук), вопросы разные, начиная от "Кто является владельцем торговой марки Linux?" и заканчивая ACL-ками в squid и маршрутизацией трафика в IPTables
...
* ну и третий этап, он очный и каждый год проводится в разных городах, там примерно задания как на втором этапе, но немного сложнее и решать их уже нужно очно, сидя с лаборатории за выделенным компьютером

Несколько лет назад было немного по-другому.
Вместо второго заочного этапа, проводились очные этапы на базе университетов.
Например для того чтобы пройти на Международный финал, нужно было сначала показать хороший результат на Всеукраинском этапе, а чтобы попасть на Всеукраинский этап, нужно в своей области показать хороший результат.
...
* задание Всеукраинского финала в Харькове в 2013 году и мой вариант решения: https://blog.amet13.name/2013/04/linux-12042013.html
* задание Международного финала в Киеве в 2013 году: https://blog.amet13.name/2013/06/linux-31052013-04062013.html
* задание 2 этапа (вроде заочное) в 2014 году: https://blog.amet13.name/2014/03/linux-it-19032014.html
* задание Международного финала в Симферополе в 2014 году: https://blog.amet13.name/2014/09/linux-20092014.html
* задание 2 этапа (заочное) в 2015 году: https://blog.amet13.name/2015/03/linux-16032015.html
...
### 2. Скрипт и управление пользователями
...
* Написать `Dockerfile` для образа `nyancat` (результаты положить в
`/home/centos/nyancatdocker/`), собрать контейнер на базе centos7
...
В домашней директории есть файл `ohmyfile`. Переместите его в `/var/`
...
Если я все верно понял, тут ничего сложного нет.
```
$ sudo -i
...
```
# exit
$ ssh -p 3119 centos@адрес_сервера
name: Amet Umerov
Edu: Sevastopol State University (already graduated in dec 2017)
IP: eth0 - 172.31.51.115
```
...
В принципе тут все понятно, ничего сложного, сначала проверяем юзера который запускает скрипт, это может делать только `root`, скачиваем файл с пользователями, сохраняем в файл, в цикле смотрим все имена пользователей в файле, генерим для каждого пароль, создаем юзера, устанавливаем пароль, доступы записываем в файл.

Запускаем скрипт и проверяем что все ок:
```
...
Устанавливаем для указанных юзеров окончание действия их аккаунтов через 30 дней:
```
...
Далее добавляем юзера в группу, чтобы он мог иметь доступ к файлам нужным и даем группе все права на все в каталоге:
```
...
Для того, чтобы юзеры могли получить общий доступ в один каталог, надо добавить их в общую группу и указать для каталога корректные права:
```
# groupadd joboffers
...
Проверим сможет ли другой юзер зайти, который не имеет отношения к каталогу:
```
# su koshermosher
$ cd /usr/share/documentstuff/
...
Все в порядке, как и задумывали.
...
Устанавливаем права на скрипт и логи, проверяем его работоспособность:
```
...
Заходим в контейнер и проверяем что ssh есть в автозагрузке, заодно и узнаем адрес виртуалки:
```
...
Все работает, можно ребутнуть тачку сейчас или в конце.
...
Качаем исходники кошки и устанавливаем:
```
...
Спасибо организаторам, за то что нет на этот раз LDAP, IPTables и прочих почтовых серверов. Их никто не любит :)

## Дополнения

* исправления --- форкайте и пульте
* вопросы можно в Issue писать
* свои варианты решений тоже можете добавить